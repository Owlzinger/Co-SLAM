...
from ac_decoder import ColorSDFNet, ColorSDFNet_v3
...
class JointEncoding(nn.Module):
    def __init__(self, config, bound_box):
        ...
        self.img2mse_uncert_alpha = (
            lambda x, y, uncert: torch.mean(
                (1 / (2 * (uncert + 1e-9).unsqueeze(-1))) * ((x - y) ** 2))
                                 + 0.5 * torch.mean(torch.log(uncert + 1e-9)) + 4.0)
    ...
    def get_decoder(self, config):
        ...
        if not self.config['grid']['oneGrid']:
            ...
            self.decoder = ColorSDFNet_v3(config, input_ch=self.input_ch, input_ch_pos=self.input_ch_pos)
        # 对应公式 2, 3 默认调用 v2
    ...
    def sdf2weights(self, sdf, z_vals, args=None):
        ...
        # [2048,84] 相邻2个SDF之间是否发生符号变化

        signs = sdf[:, 1:] * sdf[:, :-1]
        # 找到SDF符号发生变化的部分。mask设为1，否则设为0[2048, 84],# 找到SDF符号发生变化的部分。mask设为1，否则设为0  [2048,84]
    ...
    def query_sdf(self, query_points, return_geo=False, embed=False):
        ...
        W = self.decoder.sdf_net.output_dim
        sdf = self.decoder.sdf_linear(out)
        beta = self.decoder.softplus(self.decoder.uncertainty_linear(out)) + self.decoder.beta_min
        geo_feat = self.decoder.feature_linear(out)
    ...
    def render_surface_color(self, rays_o, normal):
        ...
        rgb, *_ = self.raw2outputs(raw, z_vals,
    ...
    def forward(self, rays_o, rays_d, target_rgb, target_d, global_step=0):
        ...
        if uncert.min() > 0:
            # leave alpha as 0
            # self.w是控制正则化项的权重
            rgb_loss = self.img2mse_uncert_alpha(rend_dict['rgb'] * rgb_weight, target_rgb * rgb_weight, uncert)
        else:
