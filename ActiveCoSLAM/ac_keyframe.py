...
class KeyFrameDatabase(object):
    """
    get_length方法: 返回关键帧数据库中关键帧的数量,调用__len__

    sample_single_keyframe_rays方法: 用于对当前关键帧射线进行采样,根据指定的选项（例如随机采样或深度过滤）选择射线。

    attach_ids方法: 将帧ID附加到列表中。

    add_keyframe方法: 向关键帧数据库中添加关键帧射线,根据指定的选项（例如是否进行深度过滤）对射线进行采样,并将帧ID附加到列表中。

    sample_global_rays方法: 从关键帧数据库中采样射线和帧ID,用于全局采样。

    sample_global_keyframe方法: 全局采样关键帧,限制窗口大小,并指定要固定的最后几个关键帧。

    sample_overlap_keyframe方法: 根据当前帧信息、帧ID、估计的相机到世界变换矩阵列表以及其他参数,选择与当前帧重叠的关键帧。
    """
    ...
    def add_keyframe(self, batch, filter_depth=False):
        ...
        )  # 4*368*496*(3+1+3)  给 depth 的最后加一维, 并最后一维合并
        rays = rays.reshape(1, -1, rays.shape[-1])
        # 1*(368*496)*7 只保留第一维和最后一维,中间的乘起来
    ...
    def sample_overlap_keyframe(
        ...
        target_d = (
            batch["depth"].reshape(-1, 1)[indices].repeat(1, n_samples).to(self.device)
        )
        rays_d = torch.sum(rays_d_cam[..., None, :] * c2w_est[:3, :3], -1)
        rays_o = c2w_est[None, :3, -1].repeat(rays_d.shape[0], 1).to(self.device)

        t_vals = torch.linspace(0.0, 1.0, steps=n_samples).to(target_d)
        near = target_d * 0.8
        far = target_d + 0.5
        ...
        for i, frame_id in enumerate(self.frame_ids):
            ...
            pts_flat_homo = np.concatenate([pts_flat, ones], axis=1).reshape(
                -1, 4, 1
            )  # (N, 4)
            ...
            K = np.array(
                [
                    [self.config["cam"]["fx"], 0.0, self.config["cam"]["cx"]],
                    [0.0, self.config["cam"]["fy"], self.config["cam"]["cy"]],
                    [0.0, 0.0, 1.0],
                ]
            ).reshape(3, 3)
            cam_cord[:, 0] *= -1
