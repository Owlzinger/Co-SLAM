...
def render_path(
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        print(i, time.time() - t)
        t = time.time()
        rgb, disp, acc, uncert, alpha, _ = render(
...
def render_rays(
    ...
    rgb_map, disp_map, acc_map, weights, depth_map, uncert_map, alpha_map = raw2outputs(
        raw, z_vals, rays_d, raw_noise_std, white_bkgd, pytest=pytest
    )

    if N_importance > 0:
        rgb_map_0, disp_map_0, acc_map_0, uncert_map_0, alpha_map_0 = (
            rgb_map,
            disp_map,
            acc_map,
            uncert_map,
        ...
        rgb_map, disp_map, acc_map, weights, depth_map, uncert_map, alpha_map = (
...
def choose_new_k(H, W, focal, batch_rays, k, **render_kwargs_train):
    ...
    for i in range(n):
        ...
        weight_pts = extras["weights"].reshape(
            -1, H * W, args.N_samples + args.N_importance  # 128+64
        )  # 1,160000,192
...
def config_parser():
    ...
    parser.add_argument(
        "--datadir", type=str, default="./data/fern", help="input data directory"
    )
    ...
    # dataset options
    parser.add_argument(
        "--dataset_type", type=str, default="llff", help="options: llff / blender"
    )
    parser.add_argument(
        "--testskip",
        type=int,
    ...
    parser.add_argument(
        "--i_all", type=int, default=500000
    )  # Training iterations, 500000 for full-res nerfs
    parser.add_argument(
        "--active_iter", type=int, nargs="+", default=[100000, 200000, 300000, 400000]
    )  # Iterations for active learning
    parser.add_argument(
        "--init_image", type=int, default=10
    )  # initial number of images, only for llff dataset
    parser.add_argument(
        "--choose_k", type=int, default=4
    )  # The number of new captured data for each active iter
    parser.add_argument(
        "--beta_min", type=float, default=0.01
    )  # Minimun value for uncertainty
    parser.add_argument(
        "--w", type=float, default=0.01
    )  # Strength for regularization as in Eq.(11)
    parser.add_argument(
        "--ds_rate", type=int, default=2
    )  # Quality-efficiency trade-off factor as in Sec. 5.2
...
def train():
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            rgbs, _, uncerts, alphas = render_path(
                render_poses,
                hwf,
                args.chunk,
                render_kwargs_test,
            ...
            imageio.mimwrite(
                os.path.join(testsavedir, "video_uncert.mp4"),
                to8b(uncerts / np.max(uncerts)),
                fps=30,
                quality=8,
            )
            return
    ...
    rays_rgb_all = np.transpose(
        rays_rgb_all, [0, 2, 3, 1, 4]
    )  # [N, H, W, ro+rd+rgb, 3]
    rays_rgb_train = np.stack(
    ...
    for i in trange(start, N_iters):
        ...
        if i in args.active_iter:
            ...
            print("get rays")
            rays = np.stack(
                [get_rays_np(H, W, focal, p) for p in poses.cpu().numpy()[:, :3, :4]], 0
            )  # [N, ro+rd, H, W, 3] 138*2*800*800*3 138是 138个图片(20+80+19+19). rays 是全部图像的光线
            print("done, concats")

            # get all holdout rays (candidate)
            rays_rgb_all = torch.cat(
                [torch.tensor(rays).to(device), images[:, None]], 1
            ...
            rays_rgb_holdout = torch.cat(
                [rays_rgb_all[j, :: args.ds_rate, :: args.ds_rate] for j in i_holdout],
            ...
            rays_rgb_holdout = torch.transpose(rays_rgb_holdout, 0, 1)  # 3*12800000*3 [ ro+rd+rgb,(N-1)*H*W, 3]
        ...
        dt = time.time() - time0
        print(f"Step: {global_step}, Loss: {loss}, Uncert: {uncert.mean()},Time: {dt}")
        ####           end            #####
        ...
        if i % args.i_video == 0 and i > 0:
            ...
            with torch.no_grad():
                rgbs, disps, uncerts, alphas = render_path(
